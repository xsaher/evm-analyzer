...
# Maximum times a block can be visited to prevent infinite loops in cyclic CFGs
...
def propagate_taint(graph: nx.DiGraph) -> dict[int, list[bool]]:
    ...
    while worklist:
        ...
        if visit_count[offset] >= MAX_VISITS_PER_BLOCK:
            continue
...
def check_block(instructions: list[Instruction], input_stack: list[bool], offset: int) -> list[Finding]:
    ...
    for idx, inst in enumerate(instructions):
        ...
        # Track overflow on tainted inputs incrementally (no recompute from scratch)
        if name in OVERFLOW_OPS and len(stack) >= 2:
            if any(stack[-2:]) and not has_bounds_check(instructions, idx):
                overflow_tainted = True

        if name == "SSTORE":
            if len(stack) >= 2 and any(stack[-2:]):
                severity = "HIGH"
            ...
            if len(stack) >= 3:
                addr_tainted = stack[-2]
                value_tainted = stack[-3]
